...
export default function ContentBlocksPage() {
  ...
  const load = async () => {
    ...
    setError(null)
    try {
      const res = await apiFetch<ContentBlockRow[]>("/api/admin/content-blocks")
      if (!res.ok) {
        setError(res.error)
        return
      }
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-5xl flex-col gap-6 px-6 py-10">
      <div className="flex items-center justify-between gap-4">
        <div className="text-lg font-semibold text-slate-100">Content Blocks</div>
        <button
      ...
      <GlassPanel title="Daftar (Terbaru)">
        <div className="divide-y divide-cyan-200/10">
            ...
            <div key={b.id} className="flex items-start justify-between gap-4 py-3">
              <div className="min-w-0">
                ...
                <div className="text-xs text-slate-100/70">
                  {b.scene.module.title} · {b.scene.title} ({b.scene.sceneType}) · orderNo {b.orderNo} ·{" "}
          ...
          {blocks.length === 0 ? (
            <div className="py-6 text-sm text-slate-100/70">{loading ? "Loading..." : "Belum ada content block."}</div>
          ) : null}
        </div>
      </GlassPanel>
    </main>
